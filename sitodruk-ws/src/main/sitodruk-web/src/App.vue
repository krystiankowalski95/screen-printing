    ...
    <b-nav class="navbar navbar-expand navbar-dark bg-dark">
      <div class="navbar-nav mr-auto">
        ...
        <b-nav-item href="/products"
          ><font-awesome-icon icon="gifts" />{{ $t('products') }}</b-nav-item
        >
        <b-nav-item href="/orders" v-if="isEmployeeInRole == true"
          ><font-awesome-icon icon="gifts" />{{ $t('orders') }}</b-nav-item
        >
        <b-dropdown
          v-if="isAdminInRole"
          id="dropdown"
          class="m-md-2 navbar-nav ml-auto"
        >
          <template #button-content>
            {{ $t('admin.panel') }}
          </template>
          <b-dropdown-item href="/users">{{ $t('user.list') }}</b-dropdown-item>
          <b-dropdown-item href="/createUser">{{
            $t('user.create')
          }}</b-dropdown-item>
        </b-dropdown>
      </div>
      ...
      <div v-if="currentUser" class="navbar-nav ml-auto">
        <b-dropdown id="dropdown" class="m-md-2">
          ...
          <b-dropdown-text
            ...
            <span style="font-weight: bold" v-if="isAdminInRole">{{
              $t('admin')
            }}</span>
          </b-dropdown-text>
          <b-dropdown-divider />
