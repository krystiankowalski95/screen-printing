<template>
  <div class="container">
    ...
    <b-container>
      ...
      <b-row>
        <b-col>{{ $t("price") }}</b-col>
        <b-col> <money v-model="product.price" v-bind="money" disabled name="price" /></b-col>
      </b-row>
      <b-row>
        <b-col
          ><b-button pill variant="primary" @click="edit()">{{ $t("edit") }}</b-button></b-col
        ></b-row
      >
...
export default {
  ...
  methods: {
    ...
    addProductToCart() {
      this.$store.commit("addProduct", new Product(this.product.id, this.product.name, this.product.categoryName, this.product.price, this.product.dtoVersion, this.selected));
